...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    data = np.zeros(300)
    for step1 in range(numTrials):
        virus = SimpleVirus(maxBirthProb, clearProb)
        viruses = [virus] * numViruses
        patient = Patient(viruses, maxPop)
        virus_count = []
        for step2 in range(300):
            patient.update()
            virus_count.append(patient.getTotalPop())            
        data = data + virus_count
    average = data/numTrials

    pylab.figure('viruspopulation')
    pylab.plot(average, label = 'SimpleVirus, no drugs')
    pylab.legend()
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for i in self.viruses:
            for k in self.drugResist:
                if i.resistances.get(k, 0) == False:
                    break
                else:
                    resist_pop += 1
        return resist_pop
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        data2 = data2 + resist_virus_count
    average1 = data/numTrials
    average2 = data1/numTrials
    
    pylab.xlabel('Number of steps')
    pylab.ylabel('Average Virus Population')
    pylab.title('Virus Simulation in Patient')
    pylab.plot(average1, label = 'Non-resistant population')
    pylab.plot(average2, label = 'Guttagonol Resistant population')
    pylab.legend()
    pylab.show()
