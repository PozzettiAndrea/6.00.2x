...
def generate_models(x, y, degs):
    ...
    models = []
    for d in range(degs):
        model = np.array(np.polyfit(X, Y, d+1))
        models.append(model)
    return models
...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        ...
        sums.append(sumSquaredElems)
    error = sum(sums)
    result = 1 - (error/len(y)/np.var(y))
    return result
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        ylist.append(y)
        rsq = r_squared(y, ylist)
        rsqlist.append(rsq)
    
    index = rsqlist.index(max(rsqlist))
    ...
    pylab.plot(x, ylist[index], 'r-', label = 'Fit of degree '\
                   + str(modelsDegrees[index])\
                   + ', R2 = ' + str(round(max(rsqlist), 5)))
